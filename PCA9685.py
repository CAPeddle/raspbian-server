...
class PCA9685:
  ...
  def setServoAngle(self, channel, angle):
    "Sets the Specified Servo Angle"    
    if (angle < 0  or  angle >180):
        print("Angle out of range \n")    
    else:
        temp = angle * (2000 / 180) + 500
        print ("pulse would have been {}".format(temp))
        # self.setServoPulse(channel, temp)

  def getPulse(self, angle):
        temp = angle * (2000 / 180) + 500
        return temp
  ...
  def setServoPulse(self, channel, pulse):
    ...
    try:
      ...
      if (self.debug):
        print (e)
        print ("retry casting to int {}".format(pulse))
      self.setPWM(channel, 0, math.floor(pulse))
